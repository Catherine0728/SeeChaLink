...
public class Persistence extends SQLiteOpenHelper implements BaseColumns {
	public static String TAG = "com.home.util.Persistence";
	...
	"CREATE TABLE " + TABLE_CONNECTIONS + " (" + _ID + " INTEGER PRIMARY KEY,"
			+ COLUMN_HOST + TEXT_TYPE + COMMA_SEP + COLUMN_client_ID
			+ TEXT_TYPE + COMMA_SEP + COLUMN_port + INT_TYPE + COMMA_SEP
			+ COLUMN_ssl + INT_TYPE + COMMA_SEP + COLUMN_TIME_OUT + INT_TYPE
			+ COMMA_SEP + COLUMN_KEEP_ALIVE + INT_TYPE + COMMA_SEP
			+ COLUMN_USER_NAME + TEXT_TYPE + COMMA_SEP + COLUMN_PASSWORD
			+ TEXT_TYPE + COMMA_SEP + COLUMN_CLEAN_SESSION + INT_TYPE
			+ COMMA_SEP + COLUMN_TOPIC + TEXT_TYPE + COMMA_SEP + COLUMN_MESSAGE
			+ TEXT_TYPE + COMMA_SEP + COLUMN_QOS + INT_TYPE + COMMA_SEP
	 ...
	 * 
	 * @param context
	 *            Context for creating {@link Connection} objects
	 * @return list of connections that have been restored
	 * @throws PersistenceException
	 *             if restoring connections fails, this is thrown
	 */
	public List<Connection> restoreConnections(Context context)
			throws PersistenceException {
		// columns to return
		String[] connectionColumns = { COLUMN_HOST, COLUMN_port,
				COLUMN_client_ID, COLUMN_ssl, COLUMN_KEEP_ALIVE,
		...
		for (int i = 0; i < c.getCount(); i++) {
			...
			if (null == username || "".equals(username)) {
				username = "580bbedde11605ce";
			} else {

				Log.d(TAG, "username===>0" + username);
			}
			...
			connection = Connection.createConnection(clientID, host, port,
					context, ssl);
